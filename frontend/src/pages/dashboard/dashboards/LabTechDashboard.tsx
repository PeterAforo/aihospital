...
export default function LabTechDashboard() {
  ...
  const quickActions = [
    { icon: FlaskConical, label: "Lab Worklist", path: "/lab", color: "#06b6d4" },
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={cardStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.5rem" }}>
            <span style={{ fontSize: "0.8rem", fontWeight: 500, color: "#6b7280" }}>Lab Queue</span>
            <div style={{ padding: "0.4rem", borderRadius: "8px", backgroundColor: "rgba(6,182,212,0.1)" }}>
              <FlaskConical style={{ width: 18, height: 18, color: "#06b6d4" }} />
            </div>
          </div>
          <div style={{ fontSize: "1.75rem", fontWeight: 700, color: "#1f2937" }}>—</div>
          <p style={{ fontSize: "0.7rem", color: "#9ca3af", margin: "0.25rem 0 0" }}>Visit Lab page for live worklist</p>
        </motion.div>
      </div>
