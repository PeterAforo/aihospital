...
export default function PharmacistDashboard() {
  ...
  const quickActions = [
    { icon: Pill, label: "Dispensing Queue", path: "/pharmacy", color: "#ec4899" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      <div>
        <h1 style={{ fontSize: "1.75rem", fontWeight: 700, color: "#1f2937", margin: 0 }}>Pharmacy Workspace</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={cardStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.5rem" }}>
            <span style={{ fontSize: "0.8rem", fontWeight: 500, color: "#6b7280" }}>Dispensing Queue</span>
            <div style={{ padding: "0.4rem", borderRadius: "8px", backgroundColor: "rgba(236,72,153,0.1)" }}>
              <Pill style={{ width: 18, height: 18, color: "#ec4899" }} />
            </div>
          </div>
          <div style={{ fontSize: "1.75rem", fontWeight: 700, color: "#1f2937" }}>—</div>
          <p style={{ fontSize: "0.7rem", color: "#9ca3af", margin: "0.25rem 0 0" }}>Visit Pharmacy page for live queue</p>
        </motion.div>
      </div>
