...
class StockService {
  async getStock(
    ...
    // Filter by category
    if (category) {
      result = result.filter(s => s.drug.category === category);
    }

    // Filter low stock
    if (lowStockOnly) {
      result = result.filter(s => s.quantity <= s.reorderLevel);
    }
    ...
    if (expiringDays) {
      ...
      result = result.filter(s => s.expiryDate && s.expiryDate <= expiryThreshold);
    }
  ...
  async getLowStockAlerts(tenantId: string, branchId?: string) {
    ...
    return Array.from(drugTotals.values())
      ...
      .map(d => ({
        drug: d.drug,
        currentStock: d.total,
        reorderLevel: d.reorderLevel,
        shortfall: d.reorderLevel - d.total,
      }));
  }
    ...
    switch (data.adjustmentType) {
      ...
      case 'REMOVE':
        if (balanceBefore < data.quantity) {
          throw new Error(`Cannot remove ${data.quantity}. Only ${balanceBefore} in stock.`);
        }
        balanceAfter = balanceBefore - data.quantity;
        movementQty = -data.quantity;
        break;
      case 'SET':
        ...
        movementQty = data.quantity - balanceBefore;
        break;
      default:
    ...
    if (stock) {
      await prisma.pharmacyStock.update({
        ...
        data: {
          ...
          costPrice: data.costPrice ?? stock.costPrice,
          sellingPrice: data.sellingPrice ?? stock.sellingPrice,
          expiryDate: data.expiryDate ?? stock.expiryDate,
        },
      });
    } else {
  ...
  async writeOffStock(
    ...
    if (stock.quantity < quantity) {
      throw new Error(`Cannot write off ${quantity}. Only ${stock.quantity} in stock.`);
    }
  ...
  async getStockValuation(tenantId: string, branchId?: string) {
    ...
    for (const stock of stocks) {
      ...
      items.push({
        ...
        sellingPrice: stock.sellingPrice || stock.drug.cashPrice,
        costValue,
        sellingValue,
